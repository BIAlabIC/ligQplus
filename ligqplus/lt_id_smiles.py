    ...
    for k, v in tqdm(lt_dict.items()):
        ...
        if not os.path.isfile(filepath+ os.sep +'smiles_dict_filtered'):
            ...
            for ids, smiles in smiles_dict.items():
                smiles_list.append((ids, smiles))
            lt_dict['ID']=v
            lt_dict['ID_SMILES']=smiles_list
        final_list.append(lt_dict)
    df_final_list=pd.DataFrame(final_list)
    df_final_list=df_final_list.groupby('ID')
    df_final_list2=df_final_list['ID_SMILES'].sum().apply(set).apply(list)
...
def main():
    ...
    parser.add_argument('-o', '--output', help= 'type: ORG_lt_id_smiles.csv Change ORG.', required=True)
